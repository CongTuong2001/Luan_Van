...
export const login = async (req, res) => {
  ...
  try {
    ...
    const student = await Student.findOne({ mhs: username });
    if (!user) {
...
export const createStudentAccount = async (req, res) => {
  const { username, password, ten_lop } = req.body;
...
export const deleteStudentAccount = async (req, res) => {
  try {
    const acc = await Users.findOneAndDelete({ username: req.params.mhs });
    if (acc) {
...
export const deleteTeacherAccount = async (req, res) => {
  try {
    const acc = await Users.findOneAndDelete({ username: req.params.mgv });
    if (acc) {
      res.json({ message: "Delete successfully" });
    } else {
      res.json({ message: "Delete fail" });
